...
<!--    <link rel="icon" href="assets/img/fevicon.png">-->
    ...
    <script>
        function deleteUser(id) {
            //提示
            if (confirm("您确定要删除吗？")){
                location.href=" ${pageContext.request.contextPath}/deleteUserServlet?userId="+id;
            }
        }
                    ...
                    <div style="float: left;">
                        <form class="form-inline" action="${pageContext.request.contextPath}/findUserByPageServlet">
                            <div class="form-group">
                                ...
                                <input type="text" name="user_rname" value="${condition.userRname[0]}" class="form-control" id="exampleInputName2" >
                            </div>
                            <div class="form-group">
                                <label for="exampleInputName3">公司</label>
                                <input type="text" name="user_company" value="${condition.userCompany[0]}" class="form-control" id="exampleInputName3" >
                            </div>
                            <div class="form-group">
                                <label for="exampleInputEmail2">用户名</label>
                                <input type="text" name="user_name" value="${condition.userName[0]}" class="form-control" id="exampleInputEmail2" >
                            </div>
                            <button type="submit" class="btn btn-default">查询</button>
                        </form>
                    </div>
                    <div style="float:right;margin:5px">
                        <a class="btn btn-primary" href="user/addUser_index.jsp">添加用户</a>
                    ...
                    <form id="delSelectedForm" action="${pageContext.request.contextPath}/deleteSelectedServlet" method="post" >
                        <table border="1" class="table table-bordered table-hover">
                            ...
                            <c:forEach items="${pageBean.list}" var="user" varStatus="s">
                                <tr>
                                    <td><input type="checkbox" name="selectedId" value="${user.userId}"></td>
                                    <td>${s.count}</td>
                                    <td>${user.userName}</td>
                                    ...
                                    <td>${user.userTel}</td>
                                    <td>${user.userFax}</td>
                                    <td>${user.userDatetime}</td>
                                    <td>${user.userRight}</td>
                                    <td><a class="btn btn-default btn-sm"
                                           href="${pageContext.request.contextPath}/updateFindUserServlet?userId=${user.userId}">修改</a>&nbsp;
                                        <a class="btn btn-default btn-sm"
                                           href="javascript:deleteUser(${user.userId});">删除</a></td>
                                </tr>
                            </c:forEach>
                        ...
                        <nav aria-label="Page navigation">
                            <ul class="pagination">
                                <c:if test="${pageBean.currentPage ==1}">
                                    <li class="disabled">
                                        <a href="${pageContext.request.contextPath}/findUserByPageServlet?currentPage=${pageBean.currentPage}&rows=9&userRname=${condition.userRname[0]}&userCompany=${condition.userCompany[0]}&userName=${condition.userName[0]}" aria-label="Previous">
                                ...
                                <c:if test="${pageBean.currentPage !=1}">
                                    <li>
                                        <a href="${pageContext.request.contextPath}/findUserByPageServlet?currentPage=${pageBean.currentPage-1}&rows=9&userRname=${condition.userRname[0]}&userCompany=${condition.userCompany[0]}&userName=${condition.userName[0]}" aria-label="Previous">
                                            <span aria-hidden="true">&laquo;</span>
                                        </a>
                                    </li>
                                </c:if>
                                ...
                                <c:forEach begin="1" end="${pageBean.totalPage}" var="i">
                                    <c:if test="${pageBean.currentPage ==i}">
                                        <li class="active">
                                            <a href="${pageContext.request.contextPath}/findUserByPageServlet?currentPage=${i}&rows=9&userRname=${condition.userRname[0]}&userCompany=${condition.userCompany[0]}&userName=${condition.userName[0]}">${i}</a></li>
                                    </c:if>
                                    <c:if test="${pageBean.currentPage !=i}">
                                        <li><a href="${pageContext.request.contextPath}/findUserByPageServlet?currentPage=${i}&rows=9&userRname=${condition.userRname[0]}&userCompany=${condition.userCompany[0]}&userName=${condition.userName[0]}">${i}</a></li>
                                    </c:if>
                                ...
                                <c:if test="${pageBean.currentPage ==pageBean.totalPage}">
                                    <li class="disabled">
                                        <a href="${pageContext.request.contextPath}/findUserByPageServlet?currentPage=${pageBean.currentPage}&rows=9&userRname=${condition.userRname[0]}&userCompany=${condition.userCompany[0]}&userName=${condition.userName[0]}" aria-label="Next">
                                ...
                                <c:if test="${pageBean.currentPage !=pageBean.totalPage}">
                                    <li>
                                        <a href="${pageContext.request.contextPath}/findUserByPageServlet?currentPage=${pageBean.currentPage+1}&rows=9&userRname=${condition.userRname[0]}&userCompany=${condition.userCompany[0]}&userName=${condition.userName[0]}" aria-label="Next">
                                            <span aria-hidden="true">&raquo;</span>
                                        </a>
                                    </li>
                                </c:if>
